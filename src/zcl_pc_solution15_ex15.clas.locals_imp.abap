...
*      INTO TABLE @DATA(airports).
    ...
    DATA(today) = cl_abap_context_info=>get_system_date( ). ""obtener fecha actual sistema
                                                            "Primero guardamos la fecha en una variable ABAP
                                                            "Luego la usamos dentro de select en tstmp
    SELECT
    ...
    FIELDS carrier_id, connection_id,
           ...
           div(
           tstmp_seconds_between(  "FUNCION SQL "Calcula la diferencia en segundos entre dos timestamps.
                    tstmp1 = dats_tims_to_tstmp( date = @today, time = c~departure_time, tzone = f~timzone ),
                    " Convierte una fecha, una hora y una zona horaria en un timestamp UTC único.
                    tstmp2 = dats_tims_to_tstmp( date = @today, time = c~arrival_time, tzone = t~timzone )
           )
           , 60 ) as duration
...
*      INTO TABLE @DATA(keys).
  ...
  " Comprueba si NO existe ninguna fila en flights_buffer
  " con el carrier_id solicitado.
  " Si no existe, entonces se debe leer la base de datos
  " porque aún no tenemos esos vuelos en memoria.
    SELECT
      FROM /lrn/passflight
    FIELDS carrier_id, connection_id, flight_date,
...
*           currency_code
     ...
     WHERE carrier_id = @i_carrier_id
*     ORDER BY flight_date ASCENDING "orden ascendente fecha vuelo
*    INTO TABLE @flights_buffer. "sustituir
     APPENDING TABLE @flights_buffer. "añadir
      SORT flights_buffer BY carrier_id connection_id flight_date.
      " Ordena la tabla interna flights_buffer por aerolínea, conexión y fecha.
      " Es obligatorio ordenar antes porque DELETE ADJACENT DUPLICATES
      " solo elimina duplicados que estén uno al lado del otro.
*      DELETE ADJACENT DUPLICATES FROM flights_buffer
*        COMPARING carrier_id connection_id flight_date.
*        " Elimina filas duplicadas consecutivas comparando estos tres campos.
*        " Si dos registros tienen el mismo carrier_id, connection_id y flight_date,
*        " uno de ellos será eliminado.
    ENDIF.

*    LOOP AT flights_buffer INTO DATA(flight)
*      WHERE carrier_id = i_carrier_id. " filtro
...
" Construye directamente la tabla resultado sin usar LOOP ni APPEND.

    r_result = VALUE #(
*      FOR flight IN flights_buffer "15 sustituir área de trabajo por Field Symbol -no copia <memoria
      FOR <flight> IN flights_buffer   "15 sustituir área de trabajo por Field Symbol -no copia <memoria
      WHERE ( carrier_id = i_carrier_id )
...
*                price, "Que la base de datos haga la conversión de moneda y no ABAP.
                currency_conversion(
                            amount              = price,
                            source_currency     = currency_code,
                            target_currency     = @currency, "nueva moneda
                            exchange_rate_date  = flight_date,
                on_error = @sql_currency_conversion=>c_on_error-set_to_null "¿error en conversión?
                ) AS price,
*                currency_code " la moneda original ya no es válida
                @currency AS currency_code "Necesitamos devolver la nueva moneda (la moneda destino)
         WHERE carrier_id    = @i_carrier_id
...
" Sustituir SELECT agregado por REDUCE
*    DATA total TYPE i.
*
...
*
*    SELECT
*      FROM /lrn/passflight                          " Lee datos de la tabla de vuelos de pasajeros
*      FIELDS SUM( seats_max - seats_occupied ) AS sum,  " Suma total de asientos libres (max - ocupados)
*             COUNT(*) AS count                     " Número total de vuelos encontrados
*      WHERE carrier_id = @carrier_id               " Filtra solo los vuelos de la compañía actual
*      INTO @DATA(aggregates).                      " Guarda el resultado en una estructura interna llamada 'aggregates'
*
*    r_result = aggregates-sum / aggregates-count.  " Ocupación media

  r_result =
  REDUCE #(
    INIT i = 0
    " INIT i = 0 → Se declara una variable temporal i y se inicializa a 0.
    " Esta variable servirá para acumular el total de asientos libres.

    FOR flight IN passenger_flights
    " FOR flight IN passenger_flights → Recorre cada vuelo
    " de la tabla interna passenger_flights.

    NEXT i = i + flight->get_free_seats( )
    " NEXT → En cada iteración suma a i los asientos libres
    " del vuelo actual llamando al método get_free_seats( ).
    " Es equivalente a hacer un LOOP con un acumulador.
  )
  / lines( passenger_flights ).
  " Después de calcular el total de asientos libres,
  " se divide entre el número total de vuelos.
  " Esto calcula el promedio de asientos libres.
